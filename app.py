...
def run_staffing_optimizer(results_df: pd.DataFrame) -> dict:
    ...
    if coverage_df is not None and not coverage_df.empty and st.session_state.df_input is not None:
        ...
        def _abandon_pct_row(row):
            lane = row["Lane"]
            aht_sec = float(aht_per_lane.get(lane, 0))
            if aht_sec <= 0:
                return 0.0
        ...
        if "call_volume" in cols and "Interval_Time" in cols:
            cols.remove("call_volume")
            it_pos = cols.index("Interval_Time")
            cols.insert(it_pos + 1, "call_volume")
            coverage_df = coverage_df[cols]
